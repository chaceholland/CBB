...
/**
 * ONE-TIME SCRIPT: Fetch 2024 Pitcher Rosters
 * ============================================
 * Fetches pitcher rosters from 2024 season since 2025 isn't available yet
 * 
 * Usage: node fetch-2024-rosters.cjs
 * 
 * Output: data/pitchers.json (with 2024 data)
 */
...
// Fetch 2024 roster for a team
async function fetchTeamRoster(teamId) {
  try {
    // Try regular season first (type 2)
    const url = `https://site.web.api.espn.com/apis/common/v3/sports/baseball/college-baseball/teams/${teamId}/roster?season=2024&seasontype=2`;
    ...
    // If no regular season data, try any 2024 data
    const url2 = `https://site.web.api.espn.com/apis/common/v3/sports/baseball/college-baseball/teams/${teamId}/roster?season=2024`;
    const data2 = await httpsGet(url2);
    return data2.athletes || [];
...
// Main function
async function main() {
  console.log('⚾ Fetch 2024 Pitcher Rosters (ONE-TIME SCRIPT)');
  console.log('================================================\n');
  console.log('📌 This script fetches 2024 season rosters since 2025 is not available yet.\n');
  ...
  console.log('🔍 Fetching 2024 pitcher rosters from ESPN...');
  ...
  for (const team of teams) {
    ...
    if (pitchers.length > 0) {
      ...
      console.log(`     ⚠️  No pitchers found in 2024 data`);
    }
  ...
  const output = {
    ...
    metadata: {
      ...
      source: 'ESPN API',
      season: 2024,
      note: 'Fetched from 2024 season data as 2025 rosters not yet available',
  ...
  console.log('\n✅ Done! 2024 roster data saved.');
  console.log('📌 NOTE: This is 2024 data. Update to 2025 rosters once available in February.');
}
